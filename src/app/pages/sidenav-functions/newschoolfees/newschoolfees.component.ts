...
export class NewschoolfeesComponent {
  ...
  createschoolfees(): void {
    ...
    this.http.get<number>(`https://bigezo-production.up.railway.app/api/school-fees-settings/find?year=${this.myyear}&term=${this.myterm}&level=${this.mylevel}`)
      .subscribe({
        next: (existingId) => {
          if (existingId) {
            Swal.fire({
  ...
  private createNewSchoolFeesEntry(): void {
    ...
    this.http.post<any>('https://bigezo-production.up.railway.app/api/school-fees-settings', schoolFeesSetting)
      .subscribe({
        next: (response) => {
  ...
  saveFees(): void {
    ...
    this.http.post<any>('https://bigezo-production.up.railway.app/api/school-fees-details', this.schoolFeesDetails)
      .subscribe({
        next: () => {
  ...
  deleteRemovedDetails(): void {
    ...
    this.deletedDetails.forEach((id) => {
      this.http.delete(`https://bigezo-production.up.railway.app/api/school-fees-details/${id}`)
        .subscribe({
          next: () => {
